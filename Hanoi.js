var NUM_OF_DISKS = 6 // 원반 수
var NUM_OF_COLUMNS = 3 // 움직일 수 있는 구역 수

var diskData; // 디스크 정보 
var grabData; // 잡은 디스크 정보 

var mouseLocation; // 마우스 위치


// 컬럼 초기화
var column = new Array(NUM_OF_COLUMNS); // 컬럼 배열 선언
for (var i = 0; i < column.length; i++) { // 3
  var idText = "column"+i; // 컬럼 id 값 넣을 변수 선언
  column[i] = document.getElementById(idText); // 컬럼 id 값 초기화
}
// 디스크 초기화
var disk = new Array(NUM_OF_DISKS); // 디스크 배열 선언
for (var i = 0; i < disk.length; i++) { // 5
  disk[i] = new Array(NUM_OF_COLUMNS); // 디스크의 컬럼 구역 배열 선언
}

for (var i = 0; i < disk.length; i++) { // 6
...
function grabDisk(columnNum){
  for (var i = 0; i < NUM_OF_DISKS; i++) {
    if(diskData[i][columnNum] > 0){
      ...
      // console.log(diskData[i][columnNum]);
      // console.log(grabData.diskValue); 
...
function dropDisk(columnNum){
  if(diskData[NUM_OF_DISKS-1][columnNum] == 0){ // [6-1]
    ...
    // console.log(grabData.diskValue);
    // console.log(diskData[NUM_OF_DISKS-1][columnNum]);
    return;
  }
...
function mouseClick(columnNum){
  if(grabData.isGrab == false){
    ...
    dropDisk(columnNum)
    // column[columnNum].style.backgroundColor = '#accaaf3'
  }
  draw()
}
